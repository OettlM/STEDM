...
class sViT(nn.Module):
    ...
    pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0., ns=5, t_dim=256):
        ...
        self.to_patch_embedding = SPT(dim = dim, patch_size = patch_size, channels = channels, sample_size=ns)
    ...
    def forward_single(self, img):
        # adjust shape of img to be compatible with latent diffusion
    ...
    # change name to make this the default forward function
    def forward(self, img, t_emb=None, c_old=None):
        # adjust shape of img to be compatible with latent diffusion
        img = img.permute(0,1,4,2,3)
